...
function joinCollection(ctx, entries, b1, b2) {
  ...
  if (shouldWrap) {
    ...
    ret = b1 + entries.join(',' + ctx.space) + b2;
  }
  ret.real_length = length;
  return ret;
}
...
function formatArray(ctx, value, recurseTimes) {
  ...
  for (var i = 0, l = value.length; i < l; ++i) {
    if (Object.prototype.hasOwnProperty.call(value, String(i))) {
      ...
      output.push(ctx.arrayNulls ? 'null' : '');
    }
  }
...
function formatObject(ctx, value, recurseTimes) {
  ...
  return joinCollection(ctx, output, '{' + ctx.space, ctx.space + '}');
}
...
function Formatter (config) {
  ...
  ctx.arrayNulls    = config.arrayNulls;
...
module.exports = new Formatter({
    ...
    showAccessors: false,
    format: 'jsonParity',
    arrayNulls: true, // add 'null' to array when elements are missing
    jsonParity: true,
    wrapWidth: 100,
    maxDepth: null
});
